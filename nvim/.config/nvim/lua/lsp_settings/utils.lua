...
on_attach.lsp_keymaps = function(_, bufnr)
    local function buf_set_keymap(...)
        if (bufnr) then vim.api.nvim_buf_set_keymap(bufnr, ...) end
    end

    -- Sets buffer option
    local function buf_set_option(...)
        if (bufnr) then vim.api.nvim_buf_set_option(bufnr, ...) end
    end
...
on_attach.highlight = function(client, bufnr)
    if client.server_capabilities.documentHighlightProvider then
        local augroup_id = vim.api.nvim_create_augroup(HIGHLIGHT_AUGROUP,
                                                       {clear = true})
...
local augroup_id = vim.api.nvim_create_augroup(FORMATTING_AUGROUP,
                                               {clear = true})
