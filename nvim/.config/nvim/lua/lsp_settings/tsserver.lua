...
local go_to_definition_fix = function(_, results, params)
  ...
  if vim.tbl_islist(results) then
    ...
    if #results > 1 then
      ...
      for _, result in pairs(results) do
        if string.match(result.targetUri, 'react/index.d.ts') then
          isReactDTs = true
          break
        end
      end
...
lspconfig.tsserver.setup(
  {
    ...
    handlers = { ['textDocument/definition'] = go_to_definition_fix }
  }
)
